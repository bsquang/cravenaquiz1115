...
function addQuestion(num) {
   for(var i = 0; i < num;i++){
      ...
      tempDiv +='<div class="rad_answer"><p qs-id="a" '+ (title[cate].question[i].true == 'a' ? 'istrue="true"': 'istrue="false"') +'>'+title[cate].question[i].a+'</p><i class="fa fa-check"></i><i class="fa fa-close"></i></div>';
      tempDiv +='<div class="rad_answer"><p qs-id="b" '+(title[cate].question[i].true == 'b' ? 'istrue="true"': 'istrue="false"')+'>'+title[cate].question[i].b+'</p><i class="fa fa-check"></i><i class="fa fa-close"></i></div>';
      tempDiv +='<div class="rad_answer"><p qs-id="c" '+(title[cate].question[i].true == 'c' ? 'istrue="true"': 'istrue="false"')+'>'+title[cate].question[i].c+'</p><i class="fa fa-check"></i><i class="fa fa-close"></i></div>';
      tempDiv +='<div class="rad_answer"><p qs-id="d" '+(title[cate].question[i].true == 'd' ? 'istrue="true"': 'istrue="false"')+'>'+title[cate].question[i].d+'</p><i class="fa fa-check"></i><i class="fa fa-close"></i></div>';
      tempDiv +='</div>';
      tempDiv +='<div class="next_btn1" onclick="nextQUESTION()">NEXT</div>';
      tempDiv +='</div>';
...
function chooseAnswer() {
   $('.rad_answer').click(function(){
      ...
      if (clicked == true) {
         ...
         if (select == "true") {   
            $(this).find('.fa-check').fadeIn(300).css({'color':'#00C98C'});
            trueAnswer += 1;
         }
         else{
            $(this).find('.fa-close').fadeIn(300).css({'color':'red'});
            $('.rad_answer p[istrue = "true"]').parent().find('.fa-check').fadeIn(300).css({'color':'#00C98C'});
         }
      }
   })
}
